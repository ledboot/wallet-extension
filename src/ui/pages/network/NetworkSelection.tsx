...
export default function NetworkSelection() {
  ...
  const chains =
    storedChains.length > 0
      ? storedChains
      : (Object.entries(CHAIN_INFO) as [string, ChainInfo][]);
    ...
    <div className='h-full w-full bg-base-100'>
      ...
      <div className='fixed left-0 right-0 top-0 h-14 border-b border-base-300 bg-base-100'>
        <div className='flex items-center justify-between px-4 py-3'>
          ...
          >
            <Plus className='h-7 w-7 rounded p-1 hover:bg-gray-100' />
          </div>
        </div>
      </div>
